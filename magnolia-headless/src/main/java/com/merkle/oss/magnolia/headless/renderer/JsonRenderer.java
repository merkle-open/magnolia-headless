...
public class JsonRenderer implements Renderer {
	...
	protected Map<String, Object> filter(final Map<String, Object> model) {
		return model.entrySet().stream()
				.filter(entry -> !Strings.CS.startsWith(entry.getKey(), "org.springframework.validation.BindingResult.")) //BindingResult.MODEL_KEY_PREFIX
				.filter(entry -> !Objects.equals(entry.getKey(), "springMacroRequestContext")) // AbstractTemplateView.SPRING_MACRO_REQUEST_CONTEXT_ATTRIBUTE
