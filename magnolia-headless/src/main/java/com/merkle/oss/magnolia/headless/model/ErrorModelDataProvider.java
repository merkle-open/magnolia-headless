...
public class ErrorModelDataProvider {
    ...
    public static class ErrorModel extends HashMap<String, Object> {
        public ErrorModel(final String message, final String path, final boolean editMode, final boolean throwNotEditMode) {
            super(Map.of(
                    NodeTypes.Renderable.TEMPLATE, "merkle-open_magnolia-headless:__error",
                    "msg", message,
                    "path", path,
                    "editMode", editMode,
                    "throwNotEditMode", throwNotEditMode
            ));
        }
    }
}
